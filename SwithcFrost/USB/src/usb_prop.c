...
uint8_t Report_Buf[ENDP_FEATURE_SIZE+1];
/* -------------------------------------------------------------------------- */
...
__attribute__((weak))
uint8_t HID_Feature_Send(uint8_t *Buff, uint8_t Size)
{
   return 0;
}
...
void CustomHID_Reset(void)
{
  ...
  SetEPTxCount(ENDP1, (ENDP_IN_SIZE));
  SetEPRxCount(ENDP1, (ENDP_OUT_SIZE));
...
{
#if ((ENDP_FEATURE_SIZE)>0)
    HID_Feature_Receive(Report_Buf, ENDP_FEATURE_SIZE);
#endif
}
...
#if ((ENDP_FEATURE_SIZE)>0)
    case GET_REPORT:
        if (HID_Feature_Send(Report_Buf, ENDP_FEATURE_SIZE) != 0)
        {
            CopyRoutine = CustomHID_SetReport_Feature;
            Request = GET_REPORT;
        }
...
uint8_t *CustomHID_SetReport_Feature(uint16_t Length)
{
  if (Length == 0)
  {
    pInformation->Ctrl_Info.Usb_wLength = ENDP_FEATURE_SIZE;
    return NULL;
  }
  else
  {
