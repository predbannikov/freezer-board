...
int ff;
void delay_ms2(uint32_t ms)           // милисекунды
...
void delay_uc2(uint32_t mc)           // микросекунды
{       volatile uint32_t nCount;
        RCC_ClocksTypeDef RCC_Clocks;
        RCC_GetClocksFreq (&RCC_Clocks);
...
void WriteRegisterI2C2(uint8_t RegisterAddress, uint8_t RegisterCommand)
{
...
uint8_t ReadRegisterI2C2(uint8_t RegisterAddress)
{
uint8_t data;
while(I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY));
I2C_GenerateSTART(I2C1, ENABLE);
