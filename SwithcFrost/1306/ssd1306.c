...
SSD1306_WRITECOMMAND(0xAF); //turn on scree
...
void test1306(void){
	SSD1306_GotoXY(0, 1); //Устанавливаем курсор в позицию 0;44. Сначала по горизонтали, потом вертикали.
...
SSD1306_Puts("Load weight  10kg", &Font_7x10, SSD1306_COLOR_WHITE);
		SSD1306_GotoXY(0, 16);
SSD1306_Puts("T up          -5C", &Font_7x10, SSD1306_COLOR_WHITE);
		SSD1306_GotoXY(0, 28);
SSD1306_Puts("T cntrl       +2C", &Font_7x10, SSD1306_COLOR_WHITE);
		SSD1306_GotoXY(0, 40);
SSD1306_Puts("T heater      +2C", &Font_7x10, SSD1306_COLOR_WHITE);
	SSD1306_GotoXY(0, 52);
SSD1306_Puts("Shifting   0,06mm", &Font_7x10, SSD1306_COLOR_WHITE);



	
//SSD1306_DrawCircle(120, 7, 7, SSD1306_COLOR_WHITE); //рисуем белую окружность в позиции 10;33 и радиусом 7 пикселей

//Всё, что мы нарисовали сейчас находится в буффере в оперативной памяти МК, 
	//чтобы вывести всё на дисплей необходимо вызвать:

SSD1306_UpdateScreen();
//После этого наш дисплей обновится и будет выводить надпись и кружок. 
	//После вызова SSD1306_UpdateScreen() буффер в МК не сбрасывается сам, 
	//поэтому новые рисунки будут поверх предыдущих, для сброса можно заполнить всё чёрным цветом:

SSD1306_Fill(SSD1306_COLOR_BLACK);
	
//https://habr.com/ru/articles/313490/
// https://github.com/SL-RU/sdmplayer/blob/master/src/v002/Hardware/hardware.c
}