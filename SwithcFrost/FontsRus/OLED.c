...
//	HAL_I2C_Master_Transmit(hi2c, 0x78, dt, 2, 10);
...
/*                            D R A W    S C R E E N                               */
/**********************************************************************************/
void _1306_Fill(SSD1306_COLOR_t color) {
	/* Set memory */
...
{
	uint16_t curr_dx = ABS1(x1 - x0);
	uint16_t curr_dy = ABS1(y1 - y0);
...
void OLED_ClrScreen()
{
  ...
  //SSD1306_WRITECOMMAND(0xB0 | page_number);
		...
		for (int16_t j = 0; j < OLED_WIDTH; j++)
		//{SSD1306_WRITEDATA(oled_buffer[page_number * OLED_WIDTH + j]);
		 	OLED_SendData(oled_clr_buffer[page_number * OLED_WIDTH + j]);
		}
}
...
//*********************************************
/*void displ_fill(COLOR_t color) {
	// Set memory //
	memset(oled_buffer, (color == COLOR_BLACK) ? 0x00 : 0xFF, sizeof(oled_buffer));
