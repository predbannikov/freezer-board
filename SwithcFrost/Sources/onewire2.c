...
const uint8_t convert_T2[] = {
                OW2_0, OW2_0, OW2_1, OW2_1, OW2_0, OW2_0, OW2_1, OW2_1, // 0xcc SKIP ROM
                OW2_0, OW2_0, OW2_1, OW2_0, OW2_0, OW2_0, OW2_1, OW2_0  // 0x44 CONVERT
};
const uint8_t read_scratch2[] = {
                OW2_0, OW2_0, OW2_1, OW2_1, OW2_0, OW2_0, OW2_1, OW2_1, // 0xcc SKIP ROM
                OW2_0, OW2_1, OW2_1, OW2_1, OW2_1, OW2_1, OW2_0, OW2_1, // 0xbe READ SCRATCH
                OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R,
                OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R, OW2_R
};
uint8_t scratch2[sizeof(read_scratch2)];
void OW_Init222(USART_TypeDef* USARTx) ;
...
//-----------------------------------------------------------------------------
void OW_toBits2(uint8_t ow_byte, uint8_t *ow_bits) {
	uint8_t i;
	for (i = 0; i < 8; i++) {
		if (ow_byte & 0x01) {
			*ow_bits = OW2_1;
...
// ow_bits - ссылка на буфер, размером не менее 8 байт
//-----------------------------------------------------------------------------
uint8_t OW_toByte2(uint8_t *ow_bits) {
	uint8_t ow_byte, i;
	ow_byte = 0;
	for (i = 0; i < 8; i++) {
		ow_byte = ow_byte >> 1;
...
uint8_t OW_Init2(USART_TypeDef* USARTx) {
	...
	if (OW_USART == USART1) {
		...
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE); //SR 0x000000C0
...
//-----------------------------------------------------------------------------
uint8_t OW_Reset2(USART_TypeDef* USARTx) {
//uint8_t i;
	uint8_t ow_presence;
	USART_InitTypeDef USART_InitStructure;
...
//  ow_presence = USART_ReceiveData(OW_USART);
	...
	return OW_NO_DEVICE2;
}
//---------------PA2---------------------
void OW_out_set_Power_pin9(void)
{// PA2 TX for PowerUp
...
void OW_out_Rset_Power_pin9(void)
{// PA2 TX for PowerUp
GPIO_InitTypeDef GPIO_In;
RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_AFIO,ENABLE);
...
//uint8_t i;
	...
	if (sendReset == OW_SEND_RESET2) {
		if (OW_Reset2(USARTx) == OW_NO_DEVICE2) {
			return OW_NO_DEVICE2;
		}
	}
 //
	...
	USART_DMACmd(USARTx, USART_DMAReq_Tx | USART_DMAReq_Rx, DISABLE);
		...
		if (readStart == 0 && dLen > 0) {
			*data = OW_toByte2(ow_buf2);
			data++;
			dLen--;
		   } 
      else {
			if (readStart != OW_NO_READ2) {
				readStart--;
			}
		}
	}
...
//------ Port configuring — 
GPIO_InitStructure.GPIO_Pin=GPIO_Pin_2222;
...
DMA_InitStructure.DMA_MemoryBaseAddr=(u32)&ow_buf2[0];
DMA_InitStructure.DMA_DIR=DMA_DIR_PeripheralDST;
DMA_InitStructure.DMA_BufferSize=sizeof(ow_buf2);
DMA_InitStructure.DMA_PeripheralInc=DMA_PeripheralInc_Disable;
DMA_InitStructure.DMA_MemoryInc=DMA_MemoryInc_Enable;
DMA_InitStructure.DMA_PeripheralDataSize=DMA_PeripheralDataSize_Byte;
DMA_InitStructure.DMA_MemoryDataSize=DMA_MemoryDataSize_Byte;
...
void OW_ReadData2(USART_TypeDef* USARTx, const uint8_t *command, uint8_t *buf, uint16_t len) {
        DMA_InitTypeDef DMA_InitStructure;
